...
This repository contains automation scripts to standardize the network configuration of KVM hosts running Apache CloudStack. It enforces a high-performance, segmented network topology with safety rollback mechanisms.
...
| `07_monitoring.sh` | Sets up health checks, cron jobs, and systemd timers for continuous monitoring. |
| `08_validate.sh` | Post-deployment validation script. Tests bonds, bridges, IPs, MTU, and connectivity. |
...
./02_deploy.sh --rollback
```

Backups are stored in `/var/backup/network-config/` and the last 5 backups are retained.

### Compare Configuration

Compare `server.conf` with current system state:

```bash
./02_deploy.sh --compare
```

Output shows differences in green (match) or red (mismatch).
...
This installs:
- Health check script at `/usr/local/bin/kvm-health-check.sh`
- Cron job running every 5 minutes
- Systemd timer for monitoring
- Log rotation for `/var/log/kvm-health.log`

### Manual Health Check

```bash
/usr/local/bin/kvm-health-check.sh
```

Checks:
- Bond status (slaves up/down)
- Bridge status
- IP addresses
- Gateway connectivity
- NetworkManager/libvirtd status
...
Tests:
- Bond status and redundancy
- Bridge status
- IP address assignment
- Gateway and DNS connectivity
- MTU configuration (9000 for storage, 1500 for management)
- VLAN interfaces (if tagged mode)
- Bond failover capability

### Example Output

```
=== BOND STATUS ===
[PASS] bond0 status - Mode: IEEE 802.3ad, Slaves: 2/2 UP
[PASS] bond1 status - Mode: IEEE 802.3ad, Slaves: 2/2 UP

=== BRIDGE STATUS ===
[PASS] cloudbr0 status - State: UP, Attached interfaces: 1
[PASS] cloudbr1 status - State: UP, Attached interfaces: 1
...
- **Health Check Log:** `/var/log/kvm-health.log`
- **Network Backups:** `/var/backup/network-config/`
